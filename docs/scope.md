...
The current repository has a basic Android/Kotlin app that shows read-only device notification status, lists launchable apps, searches them locally, opens Android notification settings or app info for each app, and includes an optional local audit mode. The audit mode only runs after the user enables Android notification access, uses active visible notification ranking and channel data, and keeps audit state in memory.
...
- show active behavior by app as Sound, Vibrate, Silent, None, or Unknown where Android exposes enough active-notification data
